...
/*
 ...
 Each player will recieve 5 cards each from a previously shuffled deck.
 Cards will be dealt one by one, alternating players for each card.
 The cards are stored in the players "hand" which is a vector of Card objects.
 
 Player 1 will get the 1st, 3rd, 5th, 7th & 9th card
 Player 2 will get the 2nd, 4th, 6th, 8th & 10th card
 
 Parameters	*p1 pointers to player 1 object,
 		*p2 pointers to player 2 object,
		*deck pointer to a full deck of cards
 Return		void
*/
void initialDeal(player* p1, player* p2, deck* deck)
{
    ...
    // Only exits while loop once both players have 5 cards.
    // If either player has less than 5 cards -> continue dealing cards.
...
/* 
	This method takes a player reference(pointer) and an integer to represent the cards position
	within the players hand. This method is used to find a card of the same rank as that of the 
	user selected card.
	
	
	Parameters	*currPlayer Pointer to the current player.
				cardSelect	An integer representing the position of the selected card within the players hand (Vector of Cards)
	Return		integer 
				-1 -> No card with same rank in hand
				Positive Number -> Position of card within players hand that has the same rank as the selected card.
*/
...
/* 
	Method prompts player to select a card to play from hand.
	Returns the index of the selected card in the players hand.
 
	Parameters	*currPlayer	A pointer to the current player object
	Return		Integer which is the index of the selected card within the players hand
*/
...
void sortHand(player* player)
{
    ...
    {                   // Number of sortLoops
        ...
        {
            if (player->hand[i].getRank() > player->hand[i + 1].getRank())
            {   // swap if a > b
...
void displayHand(player* player)
{
    ...
    for (int i = 1; i <= player->hand.size(); i++)
    {
        if (player->hand[i - 1].getRank() != player->hand[i].getRank())
        {   // If cards are not of same rank print large space
            cout << setw(cardWidth);
        }
        else
        {
...
bool validCard(player* currPlayer, deck* faceUp, int cardSelect, bool* burnFlag)
{
	...
	*/
    ...
    if (!faceUp->myDeck.empty())
    {                       // Must test if deck has cards otherwise .back() leads to seg fault
        if (faceUp->myDeck.back().getRank() == 3 && faceUp->myDeck.size() >= 2)
        {
            if (currPlayer->getCard(cardSelect).getRank() >= faceUp->myDeck[faceUp->myDeck.size() - 2].getRank())
            {   // 2nd last card
                return true;
            }
            else
...
int main()
{
    ...
    while (playerSet[0].hand.size() > 0 || playerSet[1].hand.size() > 0)
    {
        ...
        if (!faceUp.myDeck.empty())
        {                       // If deck is not empty
            if (faceUp.myDeck.back().getRank() == 8 && skippedFlag == false)
            {   // If top of faceUpDeck==8 >> skip
                cout << "skip" << endl;
                skippedFlag = true;
            }
            else
            {                                       // otherwise == If top of faceUpDeck!=8 >> turn
                cout << "Turn" << endl;
        ...
        {   //if the deck is empty just perform/Allow a turn >> No tests required for 8
            cout << "Turn" << endl;
            displayFaceUp(&faceUp);
            sortHand(&(playerSet[turn]));
            displayHand(&(playerSet[turn]));
    ...
    if (!playerSet[0].getNumCards())
    {   // p0 has no cards left in hand AND no cards left in deck
        cout << "p0 wins" << endl;
        cout << "ctrl-c to exit" << endl;
    }
    if (!playerSet[1].getNumCards())
    {   // p1 no cards left in hand AND no cards left in deck
        cout << "p1 wins" << endl;
        cout << "ctrl-c to exit" << endl;
    }
    return 0;
